...
import { Movie } from "constant/Genres/Movie"
    ...
    <ScrollView style={{ backgroundColor: Colors.GradTop }}>
      ...
      >
        <View style={[styles.modalContainer, { height: vh(100) - 50, paddingBottom: 40 }]}>
          ...
          />
          <ScrollView style={{ width: "100%" }}>
            {similarSeriesData ? similarSeriesData.map((movie, index) => (
              <TouchableOpacity onPress={() => {
                saveSimilarSeries(movie.seriesName, movie.seriesPoster)
              }}>
                <View key={index} style={styles.similarMovie}>
                  <Image
                    source={{ uri: movie.seriesPoster }}
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    >
                      <Text.ParagraphTitle style={{ flex: 1 }}>{movie.seriesName}</Text.ParagraphTitle>
                      <MaterialIcons
