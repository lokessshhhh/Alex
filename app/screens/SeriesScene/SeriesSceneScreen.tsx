...
const SeriesSceneScreen: NavStatelessComponent = () => {
  ...
  const editActScene = async () => {
    ...
    console.log(data)
    axios.post(BaseURl + 'series/editActScene', data, {
      headers: {
  ...
  return (
    <View style={{ backgroundColor: Colors.GradTop, height: vh(100) }}>
      <View style={styles.container}>
        ...
        <Input value={sceneName} onChangeText={(val) => setSceneName(val)} placeholder={"Scene Title"} />
        <TextInput
          value={sceneDesc}
          onChangeText={(val) => setSceneDesc(val)}
